...
class Belief(object):
    ...
    def update(self, color, sensor_position, model):
        ...
        total = 0
        for x in self.get_beliefs():
            total += self.current_distribution[x] * \
                     model.pcolorgivendist(color, utils.manhattan_distance(sensor_position, x))

        for x in self.get_beliefs():
            self.current_distribution[x] = self.current_distribution[x] * \
                                           model.pcolorgivendist(color, utils.manhattan_distance(sensor_position, x)) / total
    ...
    def recommend_sensing(self):
        ...
        highest_pos = (0, 0)
        for x in self.open:
            if highest < self.current_distribution[x]:
                highest = self.current_distribution[x]
                highest_pos = x

        if highest is 0:
            highest_pos = utils.closest_point(max(self.current_distribution.items(),
                                                  key=operator.itemgetter(1))[0], self.open)
        return highest_pos
