...
class Perceptron(object):
    ...
    def update_weights(self, feature_vector, training_label):
        ...
        predict_label = self.predict(feature_vector)
        if predict_label is not training_label:
            self.weights[predict_label] = self.weights[predict_label] - feature_vector
            self.weights[training_label] = self.weights[training_label] + feature_vector
    ...
    def predict(self, feature_vector):
        ...
        return max(self.weights, key=lambda k: self.weights[k] * feature_vector)
